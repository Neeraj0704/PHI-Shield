...
def random_ssn() -> str:
    """Generate a fake SSN (XXX-XX-XXXX). Avoid 000, 666, 9XX for area."""
    area = random.choice([x for x in range(100, 900) if x != 666 and (x // 100) != 9])
...
def random_mrn() -> str:
    """Medical record number: 6–8 digits (stored as string; display as MRN #... for redaction)."""
    return str(random.randint(100000, 99999999))


def random_phone() -> str:
    """US-style phone (555 area code for fake)."""
    return f"555-{random.randint(100, 999)}-{random.randint(1000, 9999)}"


def random_email(first_name: str, last_name: str) -> str:
    """Fake email from name."""
    domains = ["example.com", "mail.example.org", "patient-mail.test"]
    return f"{first_name.lower()}.{last_name.lower()}{random.randint(1, 99)}@{random.choice(domains)}"


def random_address() -> str:
    """Fake US address."""
    streets = ["Main St", "Oak Ave", "Maple Dr", "Cedar Ln", "Park Blvd", "Lake Rd"]
    cities = ["Springfield", "Riverside", "Fairview", "Franklin", "Clinton", "Greenville"]
    state_zip = [
        ("CA", 90001), ("TX", 75001), ("FL", 33001), ("NY", 10001), ("IL", 60001),
    ]
    state, zip_start = random.choice(state_zip)
    return f"{random.randint(1, 9999)} {random.choice(streets)}, {random.choice(cities)}, {state} {zip_start + random.randint(0, 999)}"
...
def make_patient(idx: int) -> Dict[str, Any]:
    ...
    for med_name in random.sample(MED_NAMES, k=meds_count):
        med: Dict[str, Any] = {"name": med_name}
