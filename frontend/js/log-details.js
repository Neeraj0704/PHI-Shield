...
backBtn === null || backBtn === void 0 ? void 0 : backBtn.addEventListener("click", () => window.history.back());
const riskClassMap = {
    high: "high",
    medium: "med",
    moderate: "moderate",
...
function renderLog(log) {
    var _a;
    if (logNotFound)
        logNotFound.style.display = "none";
    if (logDetailsSection)
        logDetailsSection.style.display = "block";
    if (detailTimestamp)
        detailTimestamp.textContent = formatTimestamp(log.timestamp);
    if (detailUser)
        detailUser.textContent = log.user_id || "—";
    if (detailPatientId)
        detailPatientId.textContent = log.patient_id || "—";
    const risk = ((_a = log.risk_severity) === null || _a === void 0 ? void 0 : _a.toLowerCase()) || "";
    const riskLabel = risk.charAt(0).toUpperCase() + risk.slice(1);
    const riskClass = riskClassMap[risk] || "low";
    if (detailRisk)
        detailRisk.innerHTML = `<span class="risk-badge ${riskClass}"><span class="risk-icon">⚠️</span> ${riskLabel}</span>`;
    const tags = log.phi_classifications || [];
    if (detailPhiTags) {
        if (tags.length === 0)
            detailPhiTags.textContent = "—";
        else
            detailPhiTags.innerHTML = tags.map((t) => `<span class="phi-tag">${t.type}</span>`).join(" ");
    }
    if (detailQuery)
        detailQuery.textContent = log.query || "—";
    if (detailResponse)
        detailResponse.textContent = log.llm_response || "—";
}
let currentLog = null;
function loadLog(id) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/logs/${id}`);
            if (res.status === 404) {
                if (logNotFound)
                    logNotFound.style.display = "block";
                if (logDetailsSection)
                    logDetailsSection.style.display = "none";
                return;
            }
            if (!res.ok)
                throw new Error(res.statusText);
            currentLog = yield res.json();
            if (currentLog)
