...
try {
    ...
    if (el && s.username)
        el.textContent = s.username.split("@")[0] || "User";
}
catch (_) { }
...
function userDisplay(userId) {
    ...
    if (part.toLowerCase().includes("doctor"))
        return "Dr. " + part.replace(/doctor/gi, "").trim();
...
function renderLogs(logs, filter) {
    ...
    list.forEach((log) => {
        var _a;
        const risk = riskMap[(_a = log.risk_severity) === null || _a === void 0 ? void 0 : _a.toLowerCase()] || { label: log.risk_severity || "—", class: "low" };
...
function loadLogs() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const res = yield fetch(`${API_BASE}/logs`);
            if (!res.ok)
                throw new Error(res.statusText);
            allLogs = yield res.json();
            renderLogs(allLogs);
        }
        catch (e) {
            console.error("Load logs error:", e);
            if (logsTbody)
                logsTbody.innerHTML = '<div class="row empty-row"><div>Failed to load logs. Is the backend running?</div></div>';
        }
    });
}
...
exportBtn === null || exportBtn === void 0 ? void 0 : exportBtn.addEventListener("click", () => {
    ...
    const csv = ["timestamp,user_id,patient_id,risk_severity,query"].concat(allLogs.map((l) => `"${l.timestamp}","${l.user_id}","${l.patient_id}","${l.risk_severity}","${(l.query || "").replace(/"/g, '""')}"`)).join("\n");
    const a = document.createElement("a");
    a.href = "data:text/csv;charset=utf-8," + encodeURIComponent(csv);
    a.download = "phi-access-logs.csv";
    a.click();
});
clearLogsBtn === null || clearLogsBtn === void 0 ? void 0 : clearLogsBtn.addEventListener("click", () => __awaiter(void 0, void 0, void 0, function* () {
    if (!confirm("Clear all PHI access logs? This cannot be undone. Metrics and alerts will be reset."))
        return;
    try {
        const res = yield fetch(`${API_BASE}/logs`, { method: "DELETE" });
        if (!res.ok)
            throw new Error(res.statusText);
        allLogs = [];
        renderLogs(allLogs);
